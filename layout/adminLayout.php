...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const res = await fetch("http://localhost/financial2/api/notifications_api.php?action=get_notifications");
  ...
  window.markRead = async function(id) {
    try {
      const res = await fetch("http://localhost/financial2/api/notifications_api.php?action=mark_read", {
        method: "POST",
  ...
  window.deleteNotif = async function(id) {
    try {
      const res = await fetch("http://localhost/financial2/api/notifications_api.php?action=delete", {
        method: "POST",
        body: new URLSearchParams({ id })
      });
      const data = await res.json();
  ...
  if (markAllBtn && typeof markAllBtn.addEventListener === 'function') {
    markAllBtn.addEventListener("click", async () => {
      try {
        await fetch("http://localhost/financial2/api/notifications_api.php?action=mark_read_all", { method: "POST" });
        Toastify({
...
async function logoutUser() {
    try {
  const res = await fetch('http://localhost/financial2/api/logout.php', { method: 'POST' });
        const data = await res.json();
        Toastify({ text: data.message || 'Logged out', duration: 3000 }).showToast();
        if (data.status === 'success') {
            setTimeout(() => window.location.href = 'http://localhost/financial2', 1000);
        }
    } catch (e) {
        console.error(e);
    }
